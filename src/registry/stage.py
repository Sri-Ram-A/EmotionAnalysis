...
def main():
    ...
    if not experiment: raise ValueError(f"Experiment '{EXPERIMENT_NAME}' not found.")
    ...
    if not runs: raise ValueError("No finished runs found for this experiment.")
    latest_run = runs[0]
    ...
    # print_debug_yaml(latest_run) # For debuggin purpose only
    ...
    if staged is None:
        mv = client.create_model_version(
            ...
            source=str(Path(experiment._artifact_location) / "models" / str(latest_run.outputs.model_outputs[0].model_id)),
            run_id=LATEST_RUN_ID,
        )
        client.set_registered_model_alias(EXPERIMENT_NAME, STAGED_ALIAS, mv.version)
        logger.success(f"MODEL STAGED: version = {mv.version}")

    else:
        staged_acc = client.get_run(str(staged.run_id)).data.metrics.get("val_accuracy" , -float("inf"))  
        logger.info(f"Staged model version={staged.version} | acc={staged_acc}")

        # Compare & promote if better
        if latest_acc > staged_acc:
            logger.success("Latest model is better. Promoting to staged.")
            mv = client.create_model_version(
                name=EXPERIMENT_NAME,
                source=str(Path(experiment._artifact_location) / "models" / str(latest_run.outputs.model_outputs[0].model_id)),
                run_id=LATEST_RUN_ID,
            ...
            client.set_registered_model_alias(EXPERIMENT_NAME, STAGED_ALIAS, mv.version)
            logger.success(f"Updated staged alias → version {mv.version}")
        else:
            logger.info("Staged model is already better. Keeping current staging unchanged.")
